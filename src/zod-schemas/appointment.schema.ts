...
export const createOrganizationAppointmentSchema = z
    .object({
        userUuid: z.uuid('Invalid user UUID'),
        serviceUuid: z.uuid('Invalid service UUID'),
        workerUuid: z.uuid('Invalid worker UUID'),
        roomUuid: z.uuid('Invalid room UUID'),
        scheduledStartAtUTC: z.iso.datetime({
            offset: true,
        }),
        organizationNote: z.string().trim().max(4096).nullable().optional(),
        organizationColour: colourSchema.optional(),
